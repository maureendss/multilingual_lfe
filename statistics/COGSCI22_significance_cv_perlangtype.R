...
df <- read.csv("/Users/seyssel/Work/repos/multilingual_lfe/statistics/lfe_all_cv_highdim.csv")
...
ggboxplot(df,  x ="family", y="lfe", color = "family", add = "jitter")  


ggboxplot(df,  x ="genus", y="lfe", color = "genus", add = "jitter")  
...
compare_means(lfe ~ family, data = df, paired = FALSE)
...
my_comparisons <- list( c("same", "different") )
 ggboxplot(df, x = "family", y = "lfe",
         color = "black", line.color = "gray", line.size = 0.01,
         palette = "grey", legend="none", 
         ylab="LFE score (in %)", xlab="language pair family condition",
         add = "jitter",  add.params = list(color = "black", shape=18, size=2)) +
   theme(text = element_text(size = 18))+
  stat_compare_means(paired = FALSE, comparisons=my_comparisons, label =  "p.signif", label.x = 1.5, size=7)
 ...
 ggboxplot(df, x = "family", y = "lfe",
           color = "black", line.color = "gray", line.size = 0.01,
           palette = "grey", legend="none", 
           ylab="LFE score (in %)", xlab="language family",
           add = "jitter",  add.params = list(color = "black", shape=18, size=2)) +
