...
mean_vec=false #if true, create mean_vec for train set. Only works if you use train set as test set.
...
if [ $stage -eq 5 ] || [ $stage -lt 5 ] && [ "${grad}" == "true" ]; then
        ...
        for train in $train_set; do
            ...
            for iv_type in ${train} ${test_set}; do #only in test becausez no lda
                ...
                if [ ! -f ivectors_${num_gauss}_tr-${train}${feats_suffix}_ts-${iv_type}${feats_suffix}/mean.vec ] && [ "${mean_vec}" == "true" ]; then
